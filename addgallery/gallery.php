...
class ImageManager {
    ...
    private function generateAltText($imageName) {
        ...
        return !empty($words[0]) ? $words[0] : 'gallery';
    }
    ...
    public function addImage($imageName, $file, $description = '') {
        // Validate input
        ...
        try {
            $stmt = $this->pdo->prepare("INSERT INTO images (image_name, file_path, description) VALUES (?, ?, ?)");
            $stmt->execute([$imageName, $filePath, $description]);
            return true;
        } catch(PDOException $e) {
    ...
    public function updateImage($id, $imageName, $description = '') {
        if (empty($imageName) || empty($id)) {
            throw new Exception('Image name and ID are required.');
        }
        
        try {
            $stmt = $this->pdo->prepare("UPDATE images SET image_name = ?, description = ? WHERE id = ?");
            $stmt->execute([$imageName, $description, $id]);
            return true;
        } catch(PDOException $e) {
            throw new Exception('Database error: ' . $e->getMessage());
        }
...
<head>
    ...
    <title>Golden Events - Gallery Management</title>
...
<body>
    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <h2 class="text-center mb-4 page-title">
                <i class="fas fa-images"></i> Golden Events Gallery Management System
            </h2>
            ...
            <div class="col-lg-8">
                <div class="card shadow">
                    <div class="card-header bg-success">
                        <h5 class="mb-0">
                            <i class="fas fa-list"></i> Images Gallery
                        </h5>
    ...
    <script>
        class ImageManagementApp {
            ...
            editImage(id, name, description, filePath) {
                        ...
                        <div class="col-md-6 col-lg-4 mb-4">
                            <div class="card image-card h-100">
                                <img src="${this.escapeHtml(image.file_path)}" 
                                     ...
                                     alt="${this.escapeHtml(image.alt_text || 'gallery')}">
                                <div class="card-body">
